...
Крім того, спробуєте порівняти продуктивність структур даних `OOBTree` та `dict`
для діапазонних запитів, визначаючи ефективність кожної у зберіганні та доступі
до великого обсягу даних.
...
2. Реалізуйте окремі функції для додавання товару в структуру:
   `add_item_to_tree`, `add_item_to_dict`.
...
3. `OOBTree` використовує метод `items(min, max)` для швидкого доступу до
   діапазону значень.

4. Словник `dict` реалізовує діапазонний запит за допомогою лінійного пошуку.

5. Порівняльні результати часу виконання для `OOBTree` і `dict` коректно
   виведені.
